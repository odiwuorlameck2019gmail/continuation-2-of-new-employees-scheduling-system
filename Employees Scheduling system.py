...
def Female_Employees_Absent():
    ...
    L.reshape(z,1)
    #chech if List is all ready displayed.
    if(str(L) not in scrol_Female_Employees_Absent.get('1.0',tk.END)):
       scrol_Female_Employees_Absent.insert(tk.INSERT,L)
    conn.commit()
...
labelframe.grid(column=200,row=0)
show_employees=tk.Button(labelframe,text="Absent Emplyees",font='yellow',bg='blue')
show_employees.grid(column=0,row=0,sticky='w')
scrol=scrolledtext.ScrolledText(labelframe,width=166,height=34,fg='blue',font=("arial",11))
scrol.grid(column=0,row=10)
...
def Female_Absent_employees():
    ...
    while(i<arrival_no):
        t=datetime.datetime.now()
        today=(datetime.datetime(t.year,t.month,t.day,t.hour,t.minute,t.second))
        cursor.execute("SELECT work_id FROM new_employees WHERE work_id NOT IN(SELECT work_id FROM  arrival_time) ;")
        female_absent=cursor.fetchall()
        female_absent_=female_absent
        cursor.execute("SELECT arrival_date FROM arrival_time ")  
        time_manager=cursor.fetchall()

        print(time_manager[i])
        i+=1
    ...
    cursor.execute("SELECT DISTINCT * FROM employees_absent WHERE work_id  in  (SELECT work_id FROM new_employees WHERE Gender_='Female');")
